...
class PartialPRS(object):
    ...
    * ``"A1+B1+C1"``     – full three-parameter (A, n, Ea) perturbation using
    ...
    # ====================================================================== #
    # Partial design matrix builder – DO NOT MODIFY                            #
    # ====================================================================== #
    ...
    def partial_DesignMatrix(self):
        ...
        print(
            ...
            f"\tNo. of Simulations required: {self.getSim(na, self.design)}\n"
            f"\tNo. of selected reactions: {self.selected_rxn_count}\n"
        )
        ...
        if "DesignMatrix.csv" not in os.listdir(f"DM_FOR_PARTIAL_PRS/{self.case_index}/"):
            design_matrix, selection_matrix, p_design_matrix, p_selection_matrix = (
                DM.DesignMatrix(self.unsrt, self.design, self.getSim(na, self.design), n_rxn)
                .getSample_partial(self.case_index, self.selected)
            )
            open(f"DM_FOR_PARTIAL_PRS/{self.case_index}/selected_parameters.csv", "w").write(
                self.selected_rxn_string
            )
        else:
            for rxn in self.unsrt:
        ...
        if "YAML_FILES_FOR_PARTIAL_PRS" not in os.listdir():
        ...
        if f"{self.case_index}" not in os.listdir("YAML_FILES_FOR_PARTIAL_PRS/"):
            ...
            chunk_size = 500
            params_yaml = [
                design_matrix[i : i + chunk_size]
                for i in range(0, len(design_matrix), chunk_size)
            ]
            print(self.sens_param)

            if self.sens_param == "zeta":
                params_selection_yaml = [
                    selection_matrix[i : i + chunk_size]
                    for i in range(0, len(selection_matrix), chunk_size)
                ]
            else:
                params_selection_yaml = [
                    p_selection_matrix[i : i + chunk_size]
                    for i in range(0, len(p_selection_matrix), chunk_size)
                ]
            ...
            for index, params in enumerate(params_yaml):
                ...
                location_mech = []
                index_list    = []
                for i, _ in enumerate(yaml_list):
                    index_list.append(str(count + i))
                    location_mech.append(
                        os.getcwd() + f"/YAML_FILES_FOR_PARTIAL_PRS/{self.case_index}/"
                    )
                    yaml_loc.append(
                        os.getcwd()
                        + f"/YAML_FILES_FOR_PARTIAL_PRS/{self.case_index}/mechanism_{count + i}.yaml"
                    )
                count += len(yaml_list)
            ...
            for i, _ in enumerate(design_matrix):
                ...
                yaml_loc.append(
                    os.getcwd()
                    + f"/YAML_FILES_FOR_PARTIAL_PRS/{self.case_index}/mechanism_{i}.yaml"
                )
