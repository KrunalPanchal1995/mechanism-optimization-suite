...
generate_class_B.py
===================
Generate Class-D Arrhenius perturbation samples from an XML uncertainty file.
...
How to add a new Class-D method
--------------------------------
...
2.  Register it in CLASS_D_METHOD_REGISTRY:
      CLASS_D_METHOD_REGISTRY["my_new_method"] = generate_class_D_my_new_method

3.  Call it by passing method="my_new_method" to generate_class_D_samples().
...
Note: Class-D SHGO can be slow (~10–120 s per 100 samples for m=3).
...
class ClassDSearchViz:
    """
    Live recorder + post-hoc animator for Class-D original (m=2) search.
    ...
    def _setup_live_figure(self):
        ...
        self._fig.suptitle("Class-D Search — Live", fontsize=11)
    ...
    def build_and_save(self, filename="class_b_sample.mp4", sample_id=0, folder=None):
        ...
        fig.suptitle(f"Class-D Original  |  Sample {sample_id}", fontsize=11)
...
# Sub-configurations for Class-D (no m=1)
"""
M_CONFIGS = [
    (2, "A_n",    (0, 1)),
    (2, "A_Ea",   (0, 2)),
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 4 ─ ORIGINAL Class-D INTERNAL HELPERS
# ─────────────────────────────────────────────────────────────────────────────
...
def _compute_uncorrelated_direction(L, temperatures, uncertainties):
    ...
    Used internally by the original Class-D and Class-C samplers to
...
def generate_class_D_original_full_parameters(
        ...
        n_samples, rng):
    """
    Generate one Class-D sample using the ORIGINAL published SHGO/SLSQP
    ...
    for _ in range(n_samples):
        ...
        #    filename  = f"classD_orig_sample_{_:03d}.mp4",
        #    sample_id = _,
        #    folder    = str(sub_dir)
        #)
        #if saved_path:
...
def generate_class_D_original_reduced_parameters(
        nominal, temperatures, L_full, uncertainties, indices, sub_dir,
        n_samples, rng):
    """
    Generate Class-D samples using the ORIGINAL published method adapted
...
#   2. Add it to CLASS_D_METHOD_REGISTRY below.
#   3. Call generate_class_D_samples(..., method="your_key") – done.
#
# ─────────────────────────────────────────────────────────────────────────────

CLASS_D_METHOD_REGISTRY = {
...
def generate_class_D_samples(nominal, temperatures, L_full, uncertainties, indices, sub_dir,
                              n_samples, method, rng):
    """
    Public entry point: generate n_samples Class-D zeta_r vectors.

    Dispatches to the requested algorithm via CLASS_D_METHOD_REGISTRY.
    ...
    n_samples     : number of samples to generate
    method        : key in CLASS_D_METHOD_REGISTRY, e.g. "original_m3"
    rng           : numpy Generator
    ...
    Raises
    ------
    ValueError if method is not in CLASS_D_METHOD_REGISTRY
    """
    if method not in CLASS_D_METHOD_REGISTRY:
        available = list(CLASS_D_METHOD_REGISTRY.keys())
        raise ValueError(
            f"Unknown Class-D method '{method}'. "
            f"Available: {available}"
        )
    sampler_fn = CLASS_D_METHOD_REGISTRY[method]
...
def process_reaction(rxn_tag, rxn_data, output_root, method,
    ...
    """
    Full Class-D generation pipeline for one reaction.
    ...
    2.  For every sub-config in M_CONFIGS:
          ...
          b.  Dispatch to the requested Class-D method.
    ...
    for m_level, folder, indices in M_CONFIGS:
        ...
        try:
            zeta_list = generate_class_D_samples(
        ...
        if n_generated > 0:
            ...
            plot_curves_for_subfolder(
                ...
                folder=folder, curve_class="D"   # ← change to "B" or "C"
            )
...
def main():
    ...
    print(f"[+] Class-D method : {METHOD}")
    print(f"[+] Available methods: {list(CLASS_D_METHOD_REGISTRY.keys())}\n")
