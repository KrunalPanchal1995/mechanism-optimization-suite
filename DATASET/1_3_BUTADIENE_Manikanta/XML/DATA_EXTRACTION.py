...
def getElementaryRxn(rxn,rxn_id,data):
	...
	rxn = rxn.replace(">","&#62;")
	rxn = rxn.replace(" ","&#032;")
	string = f"""\t<reaction rxn = '{rxn}' no = 'R{rxn_id}'>
...
def getDuplicateRxn(rxn,rxn_id,data):
	...
	rxn = rxn.replace(">","&#62;")
	rxn = rxn.replace(" ","&#032;")
	string = f"""\t<reaction rxn = '{rxn}' no = 'R{rxn_id}a'>
	 <class>Duplicate</class>
		...
		<temp>{temp}</temp>
		<file></file>	
	</reaction>\n\t<reaction rxn = '{rxn}' no = 'R{rxn_id}b'>
	 <class>Duplicate</class>
		<type>pressure_independent</type>
		<sub_type name = "duplicate">
			<multiple> True </multiple>
...
def getPLOGRxn(rxn,rxn_id,data,rxn_count):
	...
	rxn = rxn.replace(">","&#62;")
	rxn = rxn.replace(" ","&#032;")
	string = f"""	<PLOG rxn ='{rxn}' no = 'R{rxn_id}:High'>
	 <class>PLOG</class>
	...
	<PLOG-Interconnectedness>
	  <class>PLOG</class>
	  <InterConnectedRxns>R{rxn_id}:High,R{rxn_id}:Low</InterConnectedRxns>
...
def getPLOG_DuplicateRxn(rxn,rxn_id,data,rxn_count):
	...
	rxn = rxn.replace(">","&#62;")
	rxn = rxn.replace(" ","&#032;")
	string = f"""	<PLOG rxn = '{rxn}' no = 'R{rxn_id}a:High'>
	...
	<PLOG rxn = '{rxn}' no = 'R{rxn_id}a:Low'>
	 <class>PLOG-Duplicate</class>
		<type>pressure_dependent</type>
		<sub_type name = "forward">
			<multiple>True</multiple>
	...
	<PLOG-Interconnectedness>
	  <class>PLOG-Duplicate</class>
	  <InterConnectedRxns>R{rxn_id}a:High,R{rxn_id}a:Low</InterConnectedRxns>
	  <RxnCount>{rxn_count}</RxnCount>
	  <unsrtQuantType>Interpolation</unsrtQuantType>
	</PLOG-Interconnectedness>
	<PLOG rxn = '{rxn}' no = 'R{rxn_id}b:High'>
	 <class>PLOG-Duplicate</class>
		<type>pressure_dependent</type>
		<sub_type name = "forward">
			<multiple>True</multiple>
	...
	<PLOG rxn = 'rxn' no = 'R{rxn_id}b:Low'>
	 <class>PLOG-Duplicate</class>
		<type>pressure_dependent</type>
		<sub_type name = "forward">
			<multiple>True</multiple>
	...
	<PLOG-Interconnectedness>
	  <class>PLOG-Duplicate</class>
	  <InterConnectedRxns>R{rxn_id}b:High,R{rxn_id}b:Low</InterConnectedRxns>
	  <RxnCount>{rxn_count}</RxnCount>
	  <unsrtQuantType>Interpolation</unsrtQuantType>
	</PLOG-Interconnectedness>\n"""
	return string
